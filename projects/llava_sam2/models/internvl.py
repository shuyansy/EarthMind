    ...
    def _llm_forward(
        ...
        if fast_pixel_values is not None:
            ...
            if rgb_pixel_values is not None:
                ...
                # 1. cat sar 和 rgb
                vit_embeds = torch.cat([sar_vit_embeds, rgb_vit_embeds], dim=1)  # (B, N_total, D)

                B, N_total, D = vit_embeds.shape

                # 拆回 SAR 和 RGB
                assert N_total % 2 == 0, "Token数量必须能均分为SAR和RGB"
                half_N = N_total // 2

                sar_tokens = vit_embeds[:, :half_N, :]   # (B, N_sar, D)
                rgb_tokens = vit_embeds[:, half_N:, :]   # (B, N_rgb, D)

                # 1. local_query元素乘SAR tokens
                local_expand = self.local_query.unsqueeze(0).unsqueeze(2)  # (1, 2, 1, D)
                local_expand = local_expand.expand(B, -1, -1, -1)  # (B, 2, 1, D)

                sar_expand = sar_tokens.unsqueeze(1)  # (B, 1, N_sar, D)

                cross_modal_local_sar = local_expand * sar_expand  # (B, 2, N_sar, D)

                # 2. RGB tokens点积
                rgb_expand = rgb_tokens.unsqueeze(1)  # (B, 1, N_rgb, D)

                similarity_sar = (rgb_expand * cross_modal_local_sar).sum(dim=-1)  # (B, 2, N_sar)

                similarity_score_sar = similarity_sar.sum(dim=1)  # (B, N_sar)

                # 3. SAR importance
                sar_importance = torch.sigmoid(similarity_score_sar)  # (B, N_sar)

                # ----------------------------------------------
                # 4. 同理，对RGB tokens也做一次
                # local_query元素乘RGB tokens
                local_expand = self.local_query.unsqueeze(0).unsqueeze(2)  # (1, 2, 1, D)
                local_expand = local_expand.expand(B, -1, -1, -1)  # (B, 2, 1, D)

                rgb_expand2 = rgb_tokens.unsqueeze(1)  # (B, 1, N_rgb, D)

                cross_modal_local_rgb = local_expand * rgb_expand2  # (B, 2, N_rgb, D)

                # SAR tokens点积
                sar_expand2 = sar_tokens.unsqueeze(1)  # (B, 1, N_sar, D)

                similarity_rgb = (sar_expand2 * cross_modal_local_rgb).sum(dim=-1)  # (B, 2, N_rgb)

                similarity_score_rgb = similarity_rgb.sum(dim=1)  # (B, N_rgb)

                # 5. RGB importance
                rgb_importance = torch.sigmoid(similarity_score_rgb)  # (B, N_rgb)

                # ----------------------------------------------
                # 6. 分别reweight
                enhanced_sar_tokens = sar_tokens * sar_importance.unsqueeze(-1)  # (B, N_sar, D)
                enhanced_rgb_tokens = rgb_tokens * rgb_importance.unsqueeze(-1)  # (B, N_rgb, D)

                # 7. cat回来
                vit_embeds = torch.cat([enhanced_sar_tokens, enhanced_rgb_tokens], dim=1)  # (B, N_total, D)
