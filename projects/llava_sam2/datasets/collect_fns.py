...
def video_lisa_collate_fn(instances: Sequence[Dict],
    ...
    has_rgb_grounding_image = any(inst.get('rgb_g_pixel_values') is not None for inst in instances)
    ...
    has_rgb_image = any(inst.get('rgb_pixel_values') is not None for inst in instances)
    ...
    if has_rgb_grounding_image:
        rgb_grounding_pixel_values = []
    if has_rgb_image:
        rgb_pixel_values = []
    if has_mask:
        object_masks = []
    ...
    for example in instances:
        ...
        if has_rgb_image:
            if 'rgb_pixel_values' in example and example['rgb_pixel_values'] is not None:
                rgb_pixel_values.append(example['rgb_pixel_values'])
        if has_fast_image:
            if 'fast_pixel_values' in example and example['fast_pixel_values'] is not None:
                fast_pixel_values.append(example['fast_pixel_values'])
        if has_fast_exists:
            if 'fast_exists' in example and example['fast_exists'] is not None:
        ...
        if has_rgb_grounding_image and 'rgb_g_pixel_values' in example:
            if isinstance(example['rgb_g_pixel_values'], list):
                rgb_grounding_pixel_values += example['rgb_g_pixel_values']
            else:
                rgb_grounding_pixel_values.append(example['rgb_g_pixel_values'])
        ...
        if has_points:
            if 'points' in example and example['points'] is not None:
                prompt_points.append(example['points'])
        if has_prompt_mask:
            if 'prompt_masks' in example:
    ...
    if has_rgb_image:
        if all(x.shape == rgb_pixel_values[0].shape for x in rgb_pixel_values):
            rgb_pixel_values = torch.stack(rgb_pixel_values, dim=0)
        data_dict['rgb_pixel_values'] = rgb_pixel_values
    ...
    if has_rgb_grounding_image:
        data_dict['rgb_g_pixel_values'] = rgb_grounding_pixel_values
